    ...
    <div className="min-h-screen pt-28 pb-16 px-6 bg-[#080808]">
      <div className="max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="w-full relative overflow-hidden rounded-[1.75rem] border-2 border-dashed border-white/10 bg-white/[0.02] p-4 flex flex-col items-center justify-center min-h-[250px]">
            {uploadedUrl ? (
              <div className="relative w-full">
                <img src={uploadedUrl} alt="Preview" className="w-full max-h-72 object-cover rounded-[1.5rem]" />
                <div className="absolute top-4 right-4">
                  <button 
                    type="button" 
                    onClick={() => setUploadedUrl(null)}
                    className="bg-black/60 backdrop-blur-md border border-white/20 text-white text-[10px] px-4 py-2 rounded-full uppercase font-bold hover:bg-red-500 transition-all"
                  >
                    Remove
                  </button>
                </div>
              ...
              <UploadButton<OurFileRouter, "imageUploader">
                ...
                appearance={{
                  button: "bg-white/10 hover:bg-white/20 text-white text-xs uppercase tracking-widest px-8 py-4 rounded-xl transition-all",
                  allowedContent: "text-zinc-500 text-[10px] mt-2"
                }}
              />
            )}
          </div>
          ...
          <button type="submit" disabled={loading || !uploadedUrl}
