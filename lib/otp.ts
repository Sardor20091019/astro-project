...
// Added the missing export here so the build passes
export async function verifyOtp(email: string, inputToken: string) {
  ...
  if (!tokenRecord) throw new Error("No code found.");
  ...
  if (tokenRecord.failedAttempts >= 5) throw new Error("Too many attempts.");
